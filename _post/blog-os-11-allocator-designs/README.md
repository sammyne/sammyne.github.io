...
第 0 步展示了调整任何指针的情况。`region` 区域、`current` 区域、`region.next` 指针和 `current.next` 指针都在图中标记出来了。
...
最明显的是，释放流程也不需要遍历整个链表。这意味着调用 `dealloc` 的所需时间不会随链表长度变化。
