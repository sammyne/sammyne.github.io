...
由上可知，大部分时间都用在`append`操作上面了！好在可通过预分配切片然后给每个元素赋值的方式来简单修正。
...
+/-3% 的变动看起来不值得让我们失眠，像`encoding/json`这样的包通常会稍有加速--事实上，`encoding/json`的最新4次性能提升都小于 4%。干扰因素纷繁复杂，我们该如何确定一次变动是否真正地影响了一个包的性能呢？
