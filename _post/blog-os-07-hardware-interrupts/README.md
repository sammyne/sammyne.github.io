...
本文将会配置可编程的中断控制器，用于正确地将硬件中断导向 CPU。和之前的异常处理函数类似，为了处理这些中断，我们需要往中断描述符表添加新表项。我们将会学习如何获取周期性的时钟中断和从键盘获取输入。
...
大部分中断控制器都是可编程的，这意味着它们支持不同优先级的终端。例如，可编程使得可以赋予时钟中断比键盘中断更高的优先级，来确保准确的时钟同步。

和异常不同的是，硬件中断是 *异步* 触发的。也就是说它们完全独立于当前执行的代码，随时触发。这样使得我们的内核引入某种形式的并发，少不了潜在的并发相关 bug。Rust 严格的所有权模型这时就排上用场了，它禁止全局可修改的状态。然而，后续文章我们可以看到死锁还是可能的。
...
上图显示了中断线路的典型设置。可以看到 15 条线路中的大多数都有固定的映射，例如副 PIC 的 4 号线连接到鼠标。
...
由于会把范围在 0-15 的中断向量值传给 CPU，PIC 的默认设置用处不大。这些值已经被 CPU 异常占用了，例如 8 对应到二级异常。为了解决重叠问题，我们需要将 PIC 中断重新映射到不同的值。确切的值范文不重要，只要它们和异常的不重叠就行，但是通常会选择 32-47，因为这些是没被 32 个异常占用的首批可用值。 

配置的形式位向 PIC 的命令和数据端口写入特殊值。好在已有的 [`pic8259_simple`] 包可以排上用场，所以我们需要手码初始化流程。如果对其原理感兴趣的话，可以阅读 [其源码][pic crate source]。这是个相当小且文档注释良好的包。
...
如前面解释的那样，我们把 PIC 的值范围设为 32-47。将 `ChainedPics` 结构体包裹在 `Mutex` 使得我们可以得到安全的修改权限（借助 [`lock` 方法][spin mutex lock]），后续步骤会用上。因为传入错误的偏移会导致未定义行为，所以 `ChainedPics::new` 函数是不安全的。
...
如果一切正常的话，执行 `cargo run` 后我们应该能够继续看到 "It did not crash" 的信息。
...
## 处理时钟中断
由[上图](#the-8259-pic) 可知，计时器使用了主 PIC 的 0 号线路。这意味着到达 CPU 时的值为 32（0 + 偏移）。我们用一个 `InterruptIndex` 枚举类型存储它，而不是硬编码这个索引 32：
...
原因是 PIC 期望从中断处理函数得到一个明确的 “end of interrupt”（EOI）信号。这个信号告诉控制器中断已被处理，整个系统可以继续接收下一个中断了。所以 PIC 以为我们还在忙着处理第一个时钟中断，耐心第等待着 EOI 信号再发送下一个中断。
...
`notify_end_of_interrupt` 会确定发送中断的是主 PIC 还是副 PIC，然后使用 `command` 和 `data` 端口给相应的控制发送 EOI 信号。如果副 PIC 发送了中断，因为它是连接到主 PIC 的输入线路的，所以两个 PIC 都需要被通知。

我们需要仔细确认使用正确的中断向量值，否则有可能无意间删掉了重要的未发送中断或者导致系统卡住。这是函数为什么是不安全的原因。
...
我们使用的硬件计时器被称为 *可编程间隔计时器* 或者简称 PIT。如其名，我们可以设置两个中断的间隔。由于就快切换到 [APIC 计时器][APIC timer] 了，所以具体细节不在此深究，但是 OSDev wiki 有关于 [配置 PIT][configuring the PIT] 的大量相关文章。

## 死锁

我们的内核现在有一个一定程度的并发：计时器会触发异步中断，所以他们可以随时中断 `_start` 函数。好在 Rust 的所有权系统在编译时就可以防止多种并发相关的 bug。最著名的是死锁。如果一个线程试图获取一个绝不会被释放的锁就会触发死锁。那时线程会一直卡住。
...
我们可以在 TODO 函数的末尾循环打印一些信息，从而轻易地触发这个死锁：
...
可以看到第一个时钟中断触发前，只有有限个横杠被打印了。因为时钟中断处理函数试图打印点时被锁死了，所以整个系统卡住了。这是我们为什么没有在上面的输出看到点的原因。
...
- 为了避免另一个死锁，整个测试期间禁止中断。否则，writer 被仍被锁定期间，测试可能会被中断
- 由于时钟中断处理函数仍可能在测试运行前执行，我们在打印字符串 `s` 前打印一个新的换行符 `\n`。这样可以避免时钟处理函数已经往当前行打印了一些 `.` 字符导致测试失败的问题


执行以上更改后，`cargo test` 现在会再次确定性地成功了。

这只是个非常无害的竞争条件，置灰导致测试失败。但是可以想象，由于不确定性，调试其他竞争条件要复杂得多。好在 Rust 为我们防止了数据竞争，这类竞争是竞争条件中最为严重的一类，会触发各种未定义行为，包括系统崩溃和消无声息的内存污染。
...
我们真正想做的是在下次中断到达前挂起 CPU。这样允许 CPU 进入睡眠状态，消耗更少电量。[`hlt` 指令][`hlt` instruction] 为此而生。让我们用这个指令创建一个用电高效型无限循环把：
...
需要注意的是我们目前只讲解如何处理 [PS/2] 键盘，不包括 USB 键盘，然而，主板为了支持旧版软件会用 PS/2 设备模拟 USB 键盘，所以在内核支持 USE 之前我们可以安全地忽略 USB 键盘。
...
由 [上图](#the-8259-pic) 可知，键盘使用了主 PIC 的 1 号线路。这意味着它到达 CPU 时的值是 33（1 + 偏移 32）。我们向 `InterruptIndex` 枚举类型添加一个新的 `Keyboard` 枚举值。由于枚举值默认为前一项的值加一，也是 33，所以我们不需要显示地设置枚举值。中断处理函数中，我们打印一个 `k`，然后把中断结束信号发送给中断控制器。
...
上图显示缓慢敲入 "123" 的情形。可以看到相邻得键由邻近得扫描码，按下和释放键得扫描码是不同的。那我们得如何把扫描码翻译为确切的按键动作呢？

### 翻译扫描码

目前将扫描码映射到键有三种不同标准，即所谓的 *扫描码集*。三种标准都源自早期 IBM 电脑的键盘：[IBM XT]、[IBM 3270 PC] 和 [IBM AT]。好在后来的电脑都没有兴起定义新扫描集的风气，而是模式现有集合或拓展它们。今天的大多数键盘都可以配置为模拟上述三种集合中的任何一个：


PS/2 键盘默认模式的是扫描码集 1（“XT”）。对于这个集合，扫描码字节的低 7 位定义键值，最高有效位低位按键动作是按下（“0”）还是释放（“1”）。[IBM XT] 键盘没有定义的键，例如键盘的 enter 键，对应到两个连续的扫描码：一个 `0xe0` 转义字节，然后是一个表示键的字节。想要查看集合 1 的所有扫描码和相应的键的话，参见 [OSDev Wiki][scancode set 1]。
...
其他键的翻译流程也是一样的。好在现有一个名为 [`pc-keyboard`] 的包用于翻译扫描码集 1 和 2，所以我们不需要自己造了。为了使用这个包，把它添加到 `Cargo.toml`，然后在 `lib.rs` 中导入它：
...
为了翻译这个按键事件，我们将其传给 [`process_keyevent`] 方法，在允许条件下按键事件翻译为字符。例如，根据 shift 键是否按下，把 `A` 键的按键事件翻译为小写的 `a` 字符或大写的 `A` 字符。


有了这个更改版的中断处理函数后，我们现在就可以敲入文本了：


![在 QEMU 中敲入 "Hello World"](qemu-typing.gif)
...
本文讲解如何启用和处理外部中断。我们学习了 8259 PIC 和它的主/副布局，中断数值的重新映射和“中断结束”信号。我们位硬件时钟和键盘实现了处理函数，并学到了 `hlt` 指令，这个指令会挂起 CPU 直至下一个中断到来。

现在我们可以和内核交互，有了一些创建小型 shell 或简单游戏的基础组件。

## 下篇预告

时钟中断对操作系统是不可获取的，它们提供了周期性中断运行中进程的途径，让内核可以重获控制权。然后内核可以切换到不同进程，创造出多个进程在并行运行的假象。

但是创建进程或线程前，我们需要为他们分配内存的方法。下篇文章将会探究内核管理来实现这个基础组件。
